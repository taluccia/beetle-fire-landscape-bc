...
```
```{r}
cbigee = cbigee %>% mutate(sev_cat = ifelse(CBI_total %in% 0:0.1, "unchanged",
                                     ifelse(CBI_total %in% 0.11:1.24, "low",
                                            ifelse(CBI_total %in% 1.25:2.24, "moderate", "high"))))
head(cbigee)
```
...
##### CBI v RdNBR
```{r}
ggplot(cbigee, aes(x=CBI_total, y=rdnbr, color=sev_cat)) +
  geom_point() +
    ylab(label="RdNBR") + 
  xlab("CBI") +
...
##### CBI v dnbr
```{r}
ggplot(cbigee, aes(x=CBI_total, y=dnbr)) +
  geom_point() +
    ylab(label="RdNBR") + 
  xlab("CBI") +
  geom_hline(aes(yintercept=600), colour="darkorange2", linetype="dotdash", size=1)  +
