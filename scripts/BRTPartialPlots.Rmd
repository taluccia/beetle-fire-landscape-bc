...
```
```{r}
legend4 <- get_legend(legend.poster) 
```
...
all_mod_plots3o
### Prefire Vegetation Excluded
```{r fig.height=7, fig.width=3}
grid_PVexclude = cowplot::plot_grid(all_mod_plots3o$dndmi, all_mod_plots3o$fwi, all_mod_plots3o$elevation, legendman, ncol = 1) + 
   draw_label("no\nmortality", x = .43, y = .84, size = 10) +
   draw_label("high\nmortality", x = .83, y = .84, size = 10) +
   draw_label("moderate", x = .32, y = .59, size = 10) +
   draw_label("extreme", x = .83, y = .59, size = 10) +
   draw_label("low", x = .3, y = .34, size = 10) +
   draw_label("high", x = .84, y = .34, size = 10) +
   draw_label("(a)", x = .25, y = .98, size = 10) +
   draw_label("(b)", x = .25, y = .73, size = 10) +
   draw_label("(c)", x = .25, y = .48, size = 10) 
grid_PVexclude
```
...
```{r eval=FALSE, include=FALSE}
ggsave("../figures/2020-04-17_partialplots_outbreakONLY.tiff", plot = grid_PVexclude , width = 3, height = 7, units = c("in"), dpi=600 )
```
